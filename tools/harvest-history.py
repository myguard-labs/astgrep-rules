...
def changed_source_files(repo: Path, sha: str) -> list[tuple[str, int, int]]:
    """Return (path, added, deleted) for source files a commit touches."""
    out = run(repo, "show", "--numstat", "--format=", sha)
    files = []
    for line in out.splitlines():
        ...
        if EXCLUDE_PATH.search(path):
            continue
        files.append((path, int(added), int(deleted)))
    return files


def diff_body(repo: Path, sha: str, paths: list[str]) -> str:
    return run(repo, "show", "--format=", "--unified=6", sha, "--", *paths)


def is_cosmetic(diff: str) -> bool:
    """True when every change is whitespace-only or a pure comment edit.

    Compares the added and removed lines with all whitespace collapsed. If the
    multisets match, the commit reflowed code without changing it -- the
    'fix: apply mage format' class, which is noise for our purposes.
    """
    added, removed = [], []
...
def signal_terms(diff: str, subject: str) -> list[str]:
    ...
    return sorted(k for k, pat in terms.items() if re.search(pat, hay))
...
def harvest(repo: Path, name: str, max_files: int, max_lines: int,
    ...
    for line in shas:
        ...
        churn = sum(a + d for _, a, d in files)
        if len(files) > max_files or churn > max_lines:
            continue
        stats["sized"] += 1

        diff = diff_body(repo, sha, [p for p, _, _ in files])
        if is_cosmetic(diff):
            stats["cosmetic"] += 1
            continue
